...
class CKANHarvester(HarvesterBase):
    ...
    def gather_stage(self,harvest_job):
        ...
        log.debug('base_rest_url %s :' % base_rest_url)
        log.debug('base_search_url %s :' % base_search_url)
        if base_url.rfind('dataset') > 1:
            get_all_packages = False
            package_ids = [base_url[base_url.rfind('dataset')+8:]]

        if (previous_job and not previous_job.gather_errors and not len(previous_job.objects) == 0):
            log.info('in previou_job stage')
            if not self.config.get('force_all',False):
                ...
                try:
                    log.debug('go to url %s' % url)
                    content = self._get_content(url)
                    log.info(content)
                    revision_ids = json.loads(content)
                    log.info(revision_ids)
                    if len(revision_ids):
        ...
        if get_all_packages:
            log.debug('get_all_packages')
            # Request all remote packages
            url = base_rest_url + '/package'
            #if url.rfind('api') > 1:
            #    url = url[0:url.rfind('api')-1]
            #url = 'http://data.gov.sk/api/1/rest/package'
            try:
                content = self._get_content(url)
            except Exception,e:
                self._save_gather_error('Unable to get content for URL: %s: %s' % (url, str(e)),harvest_job)
        ...
        try:
            object_ids = []
            log.debug('try package_ids')
            if len(package_ids):
                for package_id in package_ids:
                    # Create a new HarvestObject for this identifiera
                    log.debug('package_id %s' % package_id)
    ...
    def fetch_stage(self,harvest_object):
        ...
        if url.rfind('dataset') > 1:
            url = url[0:url.rfind('dataset')-1]
